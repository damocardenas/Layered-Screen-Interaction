...
public class EETDataProviderTest : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (gazeReading != null)
        {
            ...
            RaycastHit hit;
            if (Physics.Raycast(gazeReading.EyePosition, gazeReading.GazeDirection, out hit, Mathf.Infinity, layermask))
            {
                Debug.Log(hit.collider.gameObject.name);
                hit.collider.gameObject.GetComponent<eyeDetector>().gaze_detected();
            }
        }
        /*else
        {
